...
router.get('/gateways', function(req, res, next) {
    ...
    var query_handler = backstore.dmdb_handler.collection('gw').find({}, {sort: [['did', 1]]})
    ...
    // console.log(2)
    // var  query_handler1 = backstore.dmdb_handler.collection('gw').find({laston:1505465005577},{sort:[['did',1]]})
    // query_handler1.count((err,count)=> {
    //
    //     query_handler1.skip(parseInt(req.query["offset"]))
    //         .limit(parseInt(req.query["limit"]))
    //         .toArray((err, result) => {
    //             console.log(result, "aaaaaaaaa================resulet")
    //             //res.render('gatewaylist', { dtlist: result });
    //             output = {total: count, rows: []}
    //             output["rows"] = result
    //
...
// })
    ...
    var   query_handler=backstore.dmdb_handler.collection('sl').find({},{sort:[[sort,order]]})
    query_handler.count((err,count)=>{
        query_handler.skip(parseInt(req.query["offset"]))
            .limit(parseInt(req.query["limit"]))
            .toArray((err,result)=>{
